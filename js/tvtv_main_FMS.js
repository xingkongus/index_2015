...
var attributes = {};
attributes.id = "net_tv_comment_js_plun";
attributes.name = "net_tv_comment_js_plun";
attributes.align = "middle";
swfobject.embedSWF(
	"swf/net_tv_comment_js_plun.swf", "flashContent",
	"1", "1",
...
function user_send(){
	...
	if($user_txt.replace(/[ ]/g,"")!="" && send_i==0 && $user_txt.length<200){
		...
		document.getElementById("net_tv_comment_js_plun").sendComment($u_txt);
		sr_down=true;
		send_i=1;
		setTimeout("send_i=0",0*1000) //限制只能1秒提交一次
	}
}

//检测是否加载了flash
function connected(){
	if(document.getElementById("net_tv_comment_js_plun")){
		if(user_uid!=0){
		    document.getElementById("net_tv_comment_js_plun").connectRTMP("rtmp://"+FMS_IP+"/comment",user_name); //用户登录
		    connectSucceed();
		}else{
		...
		clearTimeout(user_login);
	}
	var user_login=setTimeout("connected()",1000);
}
connected();
