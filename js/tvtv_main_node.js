...
function all_star(){//初始化函数
	...
	$('.c_live_ts').css("top",($('.c_live_box').height() - $('.live_t_control').height())-3*$('.c_live_box').height()+40);
...
function user_send(){
	...
	if($user_txt.replace(/[ ]/g,"")!="" && send_i==0 && $user_txt.length<200){
		...
		if(user_uid != "30977"){
			send_i=1;
			setTimeout("send_i=0",3*1000) //限制只能1秒提交一次
		}
	}
}
...
setTimeout("$('.c_live_ts').fadeOut(300);add_t_show.y=10;",5*1000);
